...
class SponsorCodes extends Component {
  ...
  clickRevoke(item) {
    if (!window.window.confirm("Are you sure you are going to revoke this sponsor code?"))
      return;
    this.props.dispatch(
      revokeSponsorCode(
        item.id,
  ...
  clickNew = (e) => {
    e.preventDefault();
    if (!window.window.confirm("Are you sure you are going to generate a new code?")) return;
    this.props.dispatch(
      createSponsorCode(
        () => {
          this.props.dispatch(showCanvas());
